...
HERMES_CONFIG_FILE=$(pwd)/integration-test/hermes_config.toml
...
sed -i -e 's/minimum-gas-prices = ""/minimum-gas-prices = "0ufairy"/g' $CHAIN_DIR/$CHAINID_1/config/app.toml
...
sed -i -e 's/minimum-gas-prices = ""/minimum-gas-prices = "0ufairy"/g' $CHAIN_DIR/$CHAINID_2/config/app.toml
...
sed -i -e 's/"trusted_addresses": \[\]/"trusted_addresses": \["'"$VAL1_ADDR"'","'"$VAL2_ADDR"'","'"$WALLET1_ADDR"'"\]/g' $CHAIN_DIR/$CHAINID_1/config/genesis.json
...
HKEY_1=$(hermes --config $HERMES_CONFIG_FILE keys list --chain fairyring_test_1 | sed -n '/SUCCESS/d; s/.*(\([^)]*\)).*/\1/p')
if [[ "$HKEY_1" -eq "" ]]; then
  echo "Key not found for chain id: fairyring_test_1 in Hermes Relayer Keys..."
  echo "Creating key..."
  hermes --config $HERMES_CONFIG_FILE keys add --chain fairyring_test_1 --key-file rly1.json
fi

HKEY_2=$(hermes --config $HERMES_CONFIG_FILE keys list --chain fairyring_test_2 | sed -n '/SUCCESS/d; s/.*(\([^)]*\)).*/\1/p')
if [[ "$HKEY_2" -eq "" ]]; then
  echo "Key not found for chain id: fairyring_test_1 in Hermes Relayer Keys..."
  echo "Creating key..."
  hermes --config $HERMES_CONFIG_FILE keys add --chain fairyring_test_2 --key-file rly2.json
fi
...
hermes --config $HERMES_CONFIG_FILE start > $CHAIN_DIR/relayer.log 2>&1 &
